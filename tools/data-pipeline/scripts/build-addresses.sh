...
if [ -z "$PMTILES_BIN" ]; then
  ...
  keep_published "go-pmtiles mancante"
  exit 0
fi
...
if ! DRY_RUN="$("$PMTILES_BIN" extract "$SOURCE_URL" "$Z15" --bbox="$BBOX" --minzoom=15 --maxzoom=15 --dry-run 2>&1)"; then
  echo "::warning::stima delle z15 di $REGION_ID fallita"
  keep_published "stima fallita"
  exit 0
fi
...
if [ -z "$EXTRACT_MB" ] || [ "$EXTRACT_MB" -gt "$ADDRESSES_MAX_EXTRACT_MB" ]; then
  ...
  keep_published "estrazione troppo grande"
  exit 0
fi

if ! "$PMTILES_BIN" extract "$SOURCE_URL" "$Z15" --bbox="$BBOX" --minzoom=15 --maxzoom=15; then
  echo "::warning::estrazione delle z15 di $REGION_ID fallita"
  keep_published "estrazione fallita"
  exit 0
fi
...
if [ -z "$GENERATED" ] || [ "$GENERATED" -eq 0 ]; then
  rm -f "$ADDRESSES_FILE"
  keep_published "nessun civico nelle z15"
  exit 0
fi
